<?php
	...
	require_once('db/.parametros.php');
	...
	echo ("<pre>By AC.d.C (...@gmail.com)</pre><hr/></center>");
	...
	echo("<pre><b>HOST: </b>" . host . " </pre>");
	fwrite($logfile,"HOST: " . host . "\n");
	...
	$md1 = md5($defaultadminpassword);
	$md2 = md5($HASH);
	$password = md5($md2 . $md1);

	//Abrir el script con los datos de la base de datos
	$sqlfile = file("install/install.sql", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
	
	//Conectar con la base de datos
	if($usqli)
		$session = mysqli_connect(host,
			user, password, '') or die("<h1>" . mysqli_connect_error() . "</h1>");
	else
		$session = mysqli_connect(host,
			user, password) or die("<h1>" . mysqli_connect_error() . "</h1>");
	...
	if($sqlfile)
	{
		foreach($sqlfile as $line_num => $line)
		{
			...
			if(substr($line, -1) == ';')
			{
				$tquery = str_replace("@mail", $usermail, str_replace("@pass", $password, $query));
				$timestamp = date('d/m/Y h:i:s a', time());
